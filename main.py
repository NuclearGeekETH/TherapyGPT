...
from modules.patient_form_utility import load_history, form_load, form_save, get_patient_history_text
from modules.memories_utility import load_memory, memory_load, memory_save, get_patient_memory_text
from modules.get_system_prompt import system_prompt
...
with gr.Blocks(theme=gr.themes.Soft(), title="TherapyGPT") as demo:
    ...
    with gr.Tab("Text Chat"):
        gr.Markdown("### Chat with your custom therapist")

        bot = gr.Chatbot(render=False, type='messages')

        dropdown = gr.Dropdown(
            ["gpt-4o-2024-11-20", "gpt-4.1-2025-04-14", "gemini-2.0-flash"],
            label = "Model",
        ...
        def prep_system_message():
            return f"{system_prompt}\n\nReview the patient history and start the session:\n{get_patient_history_text()} Recent Memories from past sessions: {get_patient_memory_text()}"
    ...
    with gr.Tab("Memories"):
        ...
        memories = gr.Dataframe(headers=["Timestamp","Summary","How Long Ago","Therapy Notes"], label="Previous Memories",
                                row_count=(1,"dynamic"), col_count=(4,"fixed"), datatype=["str"]*4)
        state = gr.State()
        status = gr.Markdown()
        fields = [
        ...
        demo.load(memory_load, outputs=fields)
        gr.Button("Save Changes").click(memory_save, inputs=fields, outputs=status)
